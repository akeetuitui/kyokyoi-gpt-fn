      ...
      for (const subcollection of subcollections) {
        ...
        if (!snapshot.empty) {
          snapshot.forEach((doc, index) => {
            ...
            console.log(`    - 감상문: ${data.inspiration ? `"${data.inspiration.substring(0, 50)}..." (길이: ${data.inspiration.length})` : "없음"}`);
            console.log(`    - 전시명: ${data.exhibitionTitle || data.exhibition_name || "없음"}`);
            console.log(`    - 작가명: ${data.selectedArtist || data.artistName || "없음"}`);
            console.log(`    - 방문일: ${data.visitDate || data.visit_date || data.createdAt || "없음"}`);
            console.log(`    - 전체 데이터:`, JSON.stringify(data, null, 4));
          });
        }
      }
    } else {
